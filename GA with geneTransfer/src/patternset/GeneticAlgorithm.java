...
public class GeneticAlgorithm {
    ...
    //********************************************??????????????????????????????///////////////////////////////////////
    //********************************************??????????????????????????????///////////////////////////////////////
    public static int noOfGenerationBeforeRandomRestartInGA=50; //this variable gonna only use in GA for random restart
    public static int percentOfChangeInPopulationForRandomRestart=50; 

    
    public GeneticAlgorithm(int noOfGeneration, int populationSize)
    {
    ...
    int evolve_time() throws IOException
   {
       ...
       float noOfMinToRunTheCode=Utility.timeToRunCode;
       ...
       ArrayList<Individual> maxPopulationListForRandomRestart= new ArrayList<Individual>(populationSize);     
        ...
        //************************************************int count_=1;
        while ((float)(System.currentTimeMillis()-startTime)< noOfMinToRunTheCode*5*1000)
        {         
            int countEqual=0;
...
//                 nxt.nextLine();
 ...
 public void randomRestart(int percentOfChange,ArrayList<Individual> populationList,ArrayList<Individual> maxPopulationList) 
 {
    ...
    /**
     * creating limb for limb tranfer
     * 
     */
    ArrayList<Limb> limbSet= new ArrayList<Limb>();
    if(maxPopulationList.size()>1)
    for(int i=0;i<Utility.noOfLimbInGA;i++)
    {
       int limbSize;
       do{
           limbSize=Utility.randomNoGenerator.nextInt(Utility.numberOfItems/2);//limb wont be greater than half of item
       }while(limbSize==0);
        limbSet.add(new Limb(limbSize,maxPopulationList.get(Utility.randomNoGenerator.nextInt(maxPopulationList.size()-1))));     
        //System.err.println("limb size   "+limbSize);
    }
   ...
   //initialize new node
     for(int i=0;i<noOfchange;i++)
     {
        ...
        * adding limb
        */
        Individual bestLimbIndividual= new Individual(individual);
        int limbIndex;
        for(limbIndex=0;limbIndex<limbSet.size();limbIndex++)
        {
            Individual limbIndividual= new Individual(individual);
            //System.err.println("limbindex    "+limbIndex);
            limbIndividual.addLimb((Limb)limbSet.get(limbIndex));
           
            if(limbIndividual.getIndividualScoe()>bestLimbIndividual.getIndividualScoe())
            {
               // System.out.println("size     "+limbSet.size()+"new limb  score   "+limbIndividual.getIndividualScoe()+"  old score  "+bestLimbIndividual.getIndividualScoe());
                bestLimbIndividual=new Individual(limbIndividual);
            }
        }
        individual= new Individual(bestLimbIndividual);
